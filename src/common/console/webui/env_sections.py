...
from src.common.foundation.paths import COMMON_ROOT, PROJECT_ROOT


@lru_cache(maxsize=1)
def _message_scrub_models() -> tuple[type[BaseModel], Any]:
    path = COMMON_ROOT / "features" / "message_scrub" / "config.py"
...
def _message_scrub_section() -> WebuiEnvSection:
    ...
    return WebuiEnvSection(
        id="message_scrub",
        title="消息审查与入站过滤",
        module_label="src.common.features.message_scrub",
...
def _control_plane_section() -> WebuiEnvSection:
    from src.common.features.control_plane.webui_config import ControlPlaneWebuiConfig, get_control_plane_webui_config

    return WebuiEnvSection(
        id="control_plane",
        title="联邦控制面",
        module_label="src.common.features.control_plane",
...
def _ingress_fanout_section() -> WebuiEnvSection:
    from src.common.platform.ingress.config import IngressFanoutConfig, get_ingress_fanout_config

    return WebuiEnvSection(
        id="ingress_fanout",
        title="分片：全员同响应口令",
        module_label="src.common.platform.ingress",
...
def _cmd_perm_section() -> WebuiEnvSection:
    from src.common.features.cmd_perm.config import CmdPermConfig, get_cmd_perm_config

    return WebuiEnvSection(
        id="cmd_perm",
        title="命令权限",
        module_label="src.common.features.cmd_perm",
...
def _registered_sections() -> tuple[WebuiEnvSection, ...]:
    ...
    if (COMMON_ROOT / "features" / "control_plane" / "webui_config.py").is_file():
        parts.append(_control_plane_section())
    if (COMMON_ROOT / "platform" / "ingress" / "config.py").is_file():
...
def _cmd_perm_payload_extras(cfg_obj: Any) -> dict[str, Any]:
    from src.common.features.cmd_perm.schema import build_command_perm_ui
...
def apply_webui_env_section_patch(section_id: str, patch: dict[str, Any]) -> dict[str, Any]:
    ...
    if section_id == "cmd_perm":
        try:
            from src.common.features.cmd_perm import clear_cmd_perm_cache
