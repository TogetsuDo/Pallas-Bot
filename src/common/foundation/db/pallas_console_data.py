...
async def list_all_bot_configs_public() -> list[dict[str, Any]]:
    ...
    if backend == "mongodb":
        from src.common.foundation.db.modules import BotConfigModule
    ...
    if _is_pg_backend(backend):
        ...
        from src.common.foundation.db.repository_pg import BotConfigRow, get_session
...
async def list_group_configs_public(limit: int) -> list[dict[str, Any]]:
    ...
    backend = get_db_backend()
    if backend == "mongodb":
        from src.common.foundation.db.modules import GroupConfigModule
    ...
    if _is_pg_backend(backend):
        ...
        from src.common.foundation.db.repository_pg import GroupConfigRow, get_session
...
async def list_user_configs_public(limit: int) -> list[dict[str, Any]]:
    ...
    backend = get_db_backend()
    if backend == "mongodb":
        from src.common.foundation.db.modules import UserConfigModule
    ...
    if _is_pg_backend(backend):
        ...
        from src.common.foundation.db.repository_pg import UserConfigRow, get_session
...
async def database_overview() -> dict[str, Any]:
    ...
    if _is_pg_backend(backend):
        ...
        from src.common.foundation.db.repository_pg import (
            BlackListRow,
            BotConfigRow,
            ContextRow,
            GroupConfigRow,
...
def pallas_protocol_snapshot() -> dict[str, Any] | None:
    ...
    for p in get_loaded_plugins():
        ...
        try:
            ...
            accounts = []
        try:
            from src.common.console.webui.console_login import is_console_auth_configured
