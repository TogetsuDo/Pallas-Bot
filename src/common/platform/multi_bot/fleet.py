...
def get_fleet_bot_ids() -> frozenset[int]:
    from src.common.platform.shard.data_sync import refresh_shard_data_caches_if_stale
...
def _load_fleet_bot_ids() -> set[int]:
    ...
    if is_sharding_active():
        try:
            from src.common.platform.multi_bot.session_seen import load_cluster_session_seen_ids
