...
from src.common.platform.federate.redis_settings import federate_redis_available
...
def clear_federate_config_cache() -> None:
    from src.common.platform.federate.redis_settings import clear_federate_redis_client_cache
...
def load_persisted_federate_id() -> str:
    from src.common.features.community_stats.store import load_community_stats_state
...
def federate_redis_prefix(cfg: FederateConfig | None = None) -> str:
    cfg = cfg or get_federate_config()
    try:
        from src.common.features.control_plane.store import load_bootstrap_coord_redis_prefix
