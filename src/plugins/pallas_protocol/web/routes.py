...
def register_pallas_protocol_routes(
    ...
    *,
    ...
    from src.common.console.webui.console_login import install_pallas_http_request_context_middleware
    ...
    def _render_login_page(*, target: str, err: str, detail: str) -> HTMLResponse:
        from src.common.console.webui.login_page import render_pallas_login_page_html
    ...
    async def napcat_logout(request: Request) -> RedirectResponse:  # noqa: ARG001
        from src.common.console.webui.console_login import SESSION_COOKIE_NAME
    ...
    async def protocol_change_console_login(
        ...
        cookie_token = request.cookies.get(page_cookie_name)
        _auth(x_pallas_protocol_token, token, cookie_token, request=request)
        from src.common.console.webui.console_login import set_shared_console_login_token
    ...
    async def napcat_account_workspace(
        ...
        if not manager.has_account(account_id):
            raise HTTPException(status_code=404, detail="账号不存在")
        from src.common.console.webui.console_login import extract_session_from_request
    ...
    async def nonebot_log_tail(
        ...
        def _load() -> dict[str, object]:
            from src.common.console.web import (
    ...
    async def nonebot_logs_stream(
        ...
        cookie_token = request.cookies.get(page_cookie_name)
        _auth(x_pallas_protocol_token, token, cookie_token, request=request)
        from src.common.console.web import install_nonebot_log_sink, iter_nonebot_log_sse
