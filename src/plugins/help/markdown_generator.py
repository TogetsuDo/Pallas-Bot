...
from src.common.cmd_perm import trigger_condition_with_effective_perm
    ...
    if target_plugin.metadata:
        ...
        if hasattr(metadata, "extra") and metadata.extra:
            ...
            if menu_data:
                ...
                markdown_content += "| 序号 | 功能 | 触发条件 | 简介 |\n"
                markdown_content += "|------|------|----------|------|\n"
                for i, item in enumerate(menu_data, 1):
                    func_name = _sanitize_pipe(str(item.get("func", f"未命名功能 {i}") or ""))
                    trig_raw = trigger_condition_with_effective_perm(item)
                    trig_cell = _markdown_table_cell_truncate(trig_raw, 24)
                    brief_raw = item.get("brief_des", "暂无简介") or "暂无简介"
                    brief_des = _markdown_table_cell_truncate(str(brief_raw), 20)
                    markdown_content += f"| {i} | {func_name} | {trig_cell} | {brief_des} |\n"
                markdown_content += "\n### 查看功能详情\n\n"
...
def generate_function_detail_markdown(plugin_name: str, function_name: str) -> tuple[str, HelpMarkdownIssue]:
    ...
    trigger_condition = trigger_condition_with_effective_perm(target_function)
    markdown_content += f"| 触发方式 | {_markdown_table_cell_truncate(trigger_method, 28)} |\n"
    markdown_content += f"| 触发条件 | {_markdown_table_cell_truncate(trigger_condition, 36)} |\n"
