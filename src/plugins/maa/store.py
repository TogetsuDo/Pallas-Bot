...
class MaaStore:
    ...
    async def touch_seen(self, user: str, device: str, ttl: int) -> None:
        ...
        if is_sharding_active():
            from src.common.platform.shard.coord.maa_seen_registry import touch_maa_seen_sync
    ...
    async def was_seen(self, user: str, device: str, ttl: int) -> bool:
        ...
        if is_sharding_active():
            from src.common.platform.shard.coord.maa_seen_registry import was_maa_seen_sync
    ...
    async def enqueue(
        ...
        *,
        ...
        if shard_pending:
            from src.common.platform.shard.coord.maa_pending_registry import enqueue_task_sync
    ...
    async def pending_tasks_for(self, user: str, device: str) -> list[dict[str, Any]]:
        ...
        if is_sharding_active():
            from src.common.platform.shard.coord.maa_pending_registry import list_pending_sync
    ...
    async def mark_reported(self, task_id: str) -> PendingTask | None:
        ...
        if is_sharding_active():
            from src.common.platform.shard.coord.maa_pending_registry import mark_reported_sync
    ...
    async def pending_count_for_user(self, qq_id: int) -> int:
        ...
        if is_sharding_active():
            from src.common.platform.shard.coord.maa_pending_registry import pending_count_for_user_sync
    ...
    async def pending_count_for_device(self, qq_id: int, device: str) -> int:
        ...
        if is_sharding_active():
            from src.common.platform.shard.coord.maa_pending_registry import pending_count_for_device_sync
    ...
    async def clear_pending(self, qq_id: int, *, device: str | None = None) -> int:
        ...
        if is_sharding_active():
            from src.common.platform.shard.coord.maa_pending_registry import clear_pending_sync
    ...
    async def pending_type_counts(self, qq_id: int, *, device: str | None = None) -> dict[str, int]:
        ...
        if is_sharding_active():
            from src.common.platform.shard.coord.maa_pending_registry import pending_type_counts_sync
