...
class MaaStore:
    ...
    async def pending_count_for_user(self, qq_id: int) -> int:
        user_key = str(qq_id)
        if shard_ctx.sharding_active():
            from pallas.core.platform.shard.coord.maa_pending_registry import pending_count_for_user_sync
    ...
    async def pending_count_for_device(self, qq_id: int, device: str) -> int:
        ...
        user_key = str(qq_id)
        if shard_ctx.sharding_active():
            from pallas.core.platform.shard.coord.maa_pending_registry import pending_count_for_device_sync
    ...
    async def pending_type_counts(self, qq_id: int, *, device: str | None = None) -> dict[str, int]:
        ...
        norm = normalize_device_id(device) if device else None
        if shard_ctx.sharding_active():
            from pallas.core.platform.shard.coord.maa_pending_registry import pending_type_counts_sync
