...
from .config import get_sing_config, sing_runtime_mode, sing_server_url
from .media_task_client import submit_sing_media_task
...
async def _(bot: Bot, event: GroupMessageEvent, state: T_State):
    ...
    if sing_runtime_mode(plugin_config) == "media_task":
        ok, task_id, err_text = await submit_sing_media_task(
            base_url=sing_server_url(plugin_config),
        ...
        if not ok:
            ...
            await sing_msg.finish(err_text or "我习惯了站着不动思考。有时候啊，也会被大家突然戳一戳，看看睡着了没有。")
    else:
...
async def _(bot: Bot, event: GroupMessageEvent, state: T_State):
    ...
    fail_reply = "我习惯了站着不动思考。有时候啊，也会被大家突然戳一戳，看看睡着了没有。"

    song_id = await get_song_id(song_name)
    if not song_id:
        await request_song_msg.finish(fail_reply)
        return
    ...
    await request_song_msg.finish("欢呼吧！")
