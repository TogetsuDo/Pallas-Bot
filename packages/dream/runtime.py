...
async def stop_dream_worker(bot_id: int, group_id: int) -> None:
    ...
    from pallas.core.platform.multi_bot.dedup import needs_group_host_bot_gate, release_group_owned_gate_sync
...
async def broadcast_drift(bot_id: int, source_group_id: int, payload: DriftPayload) -> None:
    ...
    from packages.dream.shard_fleet import collect_drift_peer_group_ids
...
async def launch_dream_worker(bot_id: int, group_id: int, duration_sec: int) -> None:
    ...
    from pallas.core.platform.ingress.dream_host_gate import DREAM_HOST_GATE_PLUGIN
    from pallas.core.platform.multi_bot.dedup import bind_group_owned_gate_sync, needs_group_host_bot_gate
...
async def _dream_worker_content_tick_once(
    ...
    *,
    ...
    if sent_images < image_cap and random.random() < dream_plugin_config.dream_archive_image_probability:
        ...
        draw_archive = import_plugin_submodule("draw", "draw_archive")
        for _ in range(_ARCHIVE_RESAMPLE_ATTEMPTS):
            data = await draw_archive.random_archived_png_bytes()
            if not data:
                break
