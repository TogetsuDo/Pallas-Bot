...
if TYPE_CHECKING:
    from packages.duel.duel_round_engine import LoadedEvent
...
def duel_qte_blocks_greeting_user(group_id: int, user_id: str | int) -> bool:
    ...
    if shard_ctx.sharding_active():
        from pallas.core.platform.shard.coord.duel_qte_redis import greeting_user_blocked_redis_sync
...
def pick_wrong_intrusion_name(correct: str) -> str:
    from packages.duel.arknights_ops import get_operators_payload
...
def should_schedule_bot_qte_auto_answer(responder: str) -> bool:
    ...
    if not is_fleet_bot_qq(qq):
        return False
    from pallas.core.platform.shard.presence import bot_has_local_connection
    ...
    for responder in (challenger_id, defender_id):
        ...
        async def job(responder_id: str = responder, race_coord_sid: str | None = coord_sid) -> None:
            ...
            if not success_roll or outgoing != required_key:
                return
            if race_coord_sid:
                from pallas.core.platform.shard.coord.duel_qte import try_claim_race_coord_winner
...
def apply_operator_intrusion_race_fail_outcomes(
    ...
    from packages.duel.duel_round_engine import apply_effect_dicts
...
async def _run_operator_intrusion_race_qte(
    ...
    *,
    ...
    if need_avatar:
        from packages.duel.arknights_ops import resolve_operator_avatar_image
...
def clear_all_duel_qte_sessions() -> int:
    ...
    if shard_ctx.sharding_active():
        from pallas.core.platform.shard.coord.duel_qte_redis import clear_duel_qte_greeting_redis_sync
