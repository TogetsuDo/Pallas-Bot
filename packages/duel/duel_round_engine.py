...
from packages.duel.duel_labels import bind_duel_labels, duel_label_for, reset_duel_labels, resolve_duel_labels
from packages.duel.duel_message import (
    append_duel_message,
...
from packages.duel.duel_terms import (
...
from pallas.core.limits import get_command_cooldown_sec
from pallas.core.platform.multi_bot.group import claim_group_message_event, try_acquire_group_broadcast_slot
...
async def begin_duel_command(group_id: int, *, command_id: str = "duel.duel") -> DuelCommandGate:
    """群级互斥 + 群级指令 CD。"""
    from packages.duel.duel_session import get_duel_pair
...
def reload_event_pools() -> str:
    ...
    from packages.duel.arknights_ops import get_operators_payload, reload_operators_cache
    from packages.duel.duel_qte import clear_all_duel_qte_sessions
...
def _maybe_pick_ark_ctx(ev: LoadedEvent) -> dict[str, str] | None:
    ...
    if not event_needs_ark_context(ev):
        return None
    from packages.duel.arknights_ops import (
...
async def run_exchange_bout(
    ...
    *,
    ...
    from packages.duel.duel_qte import build_exchange_auto_qte, run_event_qte_if_any
    from packages.duel.duel_send import send_duel_line
...
async def _play_clash_side(
    ...
    *,
    ...
    """单攻或单守台词。"""
    from packages.duel.duel_qte import run_event_qte_if_any
    from packages.duel.duel_send import send_duel_line
...
async def play_duel_rounds(
    ...
    *,
    ...
    from packages.duel.duel_qte import run_event_qte_if_any
    from packages.duel.duel_send import (
        ...
        for i, kind in enumerate(plan, start=1):
            ...
            try:
                ...
                if kind == "public":
                    ...
                    from packages.duel.duel_send import round_buffer_prepend, send_duel_line
