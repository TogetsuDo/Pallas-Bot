...
async def test_build_corpus_status_snapshot_shape(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("src.common.corpus.status.corpus_community_enrollment_valid", lambda _state=None: True)
    monkeypatch.setattr("src.common.corpus.status.community_manual_configured", lambda: False)
    monkeypatch.setattr("src.common.corpus.status.fed_configured", lambda: False)
    monkeypatch.setattr(
