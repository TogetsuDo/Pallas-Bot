def test_community_contribute_enabled_defaults_true(monkeypatch):
    from src.common.features.corpus.config import clear_corpus_config_cache, community_contribute_enabled

    monkeypatch.delenv("PALLAS_CORPUS_COMMUNITY_CONTRIBUTE", raising=False)
    monkeypatch.setattr(
        "src.common.features.corpus.config.setting_str",
        lambda name, default="": default if name == "PALLAS_CORPUS_COMMUNITY_CONTRIBUTE" else "",
    )
    monkeypatch.setattr("src.common.features.corpus.store.load_corpus_community_state", dict)
...
def test_community_contribute_respects_enroll_policy_false(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.common.features.corpus.store.load_corpus_community_state",
