...
from src.platform.shard.logs.errors import errors_jsonl_path
from src.platform.shard.logs.session import (
...
def test_rotate_creates_fresh_main_log(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("src.platform.shard.logs.view.shard_logs_dir", logs_dir)
    monkeypatch.setattr("src.platform.shard.logs.session.shard_logs_dir", logs_dir)
    monkeypatch.setattr("src.platform.shard.logs.errors.shard_logs_dir", logs_dir)
...
def test_rotate_skipped_when_disabled(tmp_path, monkeypatch):
    ...
    main.write_text("old\n", encoding="utf-8")
    monkeypatch.setattr("src.platform.shard.logs.view.shard_logs_dir", lambda: log_dir)
    monkeypatch.setattr("src.platform.shard.logs.session.shard_logs_dir", lambda: log_dir)
