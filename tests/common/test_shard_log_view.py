...
from src.common.platform.shard.logs.view import (
...
from src.common.platform.shard.registry.store import ShardRecord, ShardRegistry, TestShardConfig
...
def test_list_shard_log_sources_skips_orphan_worker_log(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("src.common.platform.shard.registry.store.get_shard_registry", lambda: reg)
...
def test_collect_cluster_log_errors_no_log_rescan_after_cleanup(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("src.common.platform.shard.logs.view.shard_logs_dir", lambda: log_dir)
    monkeypatch.setattr("src.common.platform.shard.logs.errors.shard_errors_dir", lambda: err_dir)
...
def test_collect_cluster_log_errors(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("src.common.platform.shard.logs.view.shard_logs_dir", lambda: log_dir)
    monkeypatch.setattr("src.common.platform.shard.logs.errors.shard_logs_dir", lambda: log_dir)
    rows = collect_cluster_log_errors(per_file=50, limit=10)
    assert len(rows) >= 1
