...
from pallas.core.platform.ingress.fanout_bypass import is_greeting_fanout_plaintext, is_ingress_fanout_plaintext
...
def test_fanout_texts_default(monkeypatch):
    monkeypatch.setattr(
        ...
        lambda name, default="": (
            "牛牛,帕拉斯,牛牛报数,牛牛出列" if name == "PALLAS_INGRESS_FANOUT_GREETING" else default
        ),
    )
    clear_ingress_fanout_config_cache()
