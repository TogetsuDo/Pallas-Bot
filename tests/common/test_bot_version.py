...
from src.common.community_stats.reporter import build_heartbeat_payload
...
def test_get_pallas_bot_version_for_reporting_prefers_exact_tag():
    with patch(
        "src.common.bot_version.get_bot_current_version",
...
def test_get_pallas_bot_version_for_reporting_falls_back_to_health():
    with (
        patch("src.common.bot_version.get_bot_current_version", return_value={"tag": "", "commit": "abc1234"}),
        patch("src.common.bot_version.get_pallas_bot_version_for_health", return_value="v3.0.0-12-gdeadbeef"),
    ):
...
def test_build_heartbeat_payload_uses_runtime_version():
    with (
        patch(
            "src.common.community_stats.reporter.get_pallas_bot_version_for_reporting",
            return_value="v3.1.0",
        ),
        patch(
