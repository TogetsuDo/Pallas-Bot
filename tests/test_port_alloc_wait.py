...
def test_ports_still_in_use_dedupes() -> None:
    with patch("src.platform.shard.registry.port_alloc.is_tcp_port_in_use", side_effect=[True, False]):
        assert ports_still_in_use([8090, 8090, 8091]) == [8090]


def test_wait_tcp_ports_free_immediate() -> None:
    with patch("src.platform.shard.registry.port_alloc.ports_still_in_use", return_value=[]):
...
def test_wait_tcp_ports_free_timeout() -> None:
    with patch("src.platform.shard.registry.port_alloc.ports_still_in_use", return_value=[8090]):
