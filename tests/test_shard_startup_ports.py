...
def test_evaluate_skips_registry_when_nominal_ports_occupied(tmp_path, monkeypatch):
    env_path = tmp_path / ".env"
    env_path.write_text(
        "PALLAS_SHARD_ENABLED=true\nPALLAS_SHARD_WORKER_BASE_PORT=8090\nPALLAS_SHARD_WS_HOST=127.0.0.1\n",
        encoding="utf-8",
    ...
    from pallas.core.platform.shard.registry.startup_ports import evaluate_registry_worker_ports

    with patch("pallas.core.platform.shard.registry.port_alloc.is_tcp_port_in_use", return_value=True):
        ev = evaluate_registry_worker_ports(1, 8090, env_path=env_path, skip_occupied=True)
    assert ev.skip_registry_alloc is True
    assert ev.worker_ports == [8090]
    assert any("运行中" in note for note in ev.notes)
