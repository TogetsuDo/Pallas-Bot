...
def test_shard_ids(reg: ShardRegistry) -> set[int]:
    return {int(s.id) for s in reg.shards if is_test_shard_record(s, reg)}


def _normal_worker_need(reg: ShardRegistry) -> int:
    test_ids = test_shard_ids(reg)
    normal_assigns = [int(v) for v in reg.assignments.values() if int(v) not in test_ids]
...
def _ensure_shard_rows(reg: ShardRegistry) -> None:
    ...
    for sid, row in by_id.items():
        if sid in kept or sid == test_sid:
            continue
        if (row.role or "").strip().lower() == TEST_SHARD_ROLE:
            kept[sid] = row
            continue
        if is_test_shard_record(row, reg):
            continue
...
def assign_bot_to_shard(bot_id: str, *, registry: ShardRegistry | None = None) -> int:
    ...
    save_shard_registry(reg)
    try:
        from pallas.core.platform.shard.worker_scale import schedule_worker_scale_restart
