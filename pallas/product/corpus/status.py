...
from pallas.product.corpus.store import corpus_community_enrollment_valid, load_corpus_community_state
...
async def build_corpus_status_snapshot() -> dict[str, Any]:
    ...
    try:
        from pallas.product.control_plane.config import get_control_plane_config, should_run_bootstrap_refresh
        from pallas.product.control_plane.store import (
            bootstrap_state_valid,
            load_bootstrap_coord_redis_url,
        )
        from pallas.product.control_plane.webui_config import get_control_plane_webui_config

        cp_cfg = get_control_plane_config()
        cp_web = get_control_plane_webui_config()
        bootstrap_fid = ""
        bootstrap_expires: int | None = None
        bootstrap_corpus_raw: dict[str, Any] | None = None
        try:
            from pallas.product.community_stats.store import _read_state_raw
            ...
            if isinstance(boot, dict):
                ...
                if isinstance(boot.get("corpus_community"), dict):
                    bootstrap_corpus_raw = dict(boot["corpus_community"])
            bootstrap_fid = str(raw.get("federate_id") or "").strip()
        except Exception:
            pass
        bootstrap_corpus_out: dict[str, Any] | None = None
        if bootstrap_corpus_raw is not None:
            bootstrap_corpus_out = {
                "api_base": str(bootstrap_corpus_raw.get("api_base") or "").strip().rstrip("/"),
                "readable": bool(bootstrap_corpus_raw.get("readable")),
                "writable": bool(bootstrap_corpus_raw.get("writable")),
            }
        ...
        control_plane = {
            ...
            "coord_redis_configured": bool((cp_web.coord_redis_url or "").strip() or load_bootstrap_coord_redis_url()),
            ...
            "bootstrap_corpus_community": bootstrap_corpus_out,
        }
    except Exception:
        control_plane = {}
    ...
    if community_on and (enrolled or manual) and token:
        try:
            from pallas.product.corpus.usage import fetch_corpus_community_usage
