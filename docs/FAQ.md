...
不知道从哪查？先走下面决策树。  
专题入口：[五分钟跑起来](guide/quickstart.md) · [运维入口](maintainer/quickstart.md) · [排障](maintainer/operate/troubleshooting.md) · [配置要点](Config.md)
...
A: **不能沿用旧路径。** 4.0 已移除 `src/` 目录；内置玩法迁至 **`packages/`** 与 **`pallas-plugin-*` 官方插件**，社区插件应使用 **`pallas.api.*`** 与 **`packages/<name>/` 或 `local/plugins/`** 布局。从 3.x 升级请读 [4.0 迁移指南](guide/4.0-migration.md)。
