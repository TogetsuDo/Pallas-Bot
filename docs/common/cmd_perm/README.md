...
实现代码位于 `src/common/features/cmd_perm/`。
...
- 文件：`src/common/features/cmd_perm/registry.py` 中 `DEFAULT_COMMAND_PERMISSIONS`。
...
`PluginMetadata` 共用常量：`src/common/features/cmd_perm/metadata_defaults.py`（`PLUGIN_HOMEPAGE`、`PLUGIN_EXTRA_VERSION`、`PLUGIN_MENU_TEMPLATE`）。
...
| 路径 | 职责 |
|------|------|
| `src/common/features/cmd_perm/check.py` | `permission_for_command`、`group_message_permission_for_command`、`private_message_permission_for_command`、`satisfies_command_permission` |
| `src/common/features/cmd_perm/config.py` | 从环境读取覆盖、`get_cmd_perm_config`、`clear_cmd_perm_cache` |
| `src/common/features/cmd_perm/registry.py` | 合法等级、`DEFAULT_COMMAND_PERMISSIONS`、`resolved_level` |
| `src/common/features/cmd_perm/schema.py` | 合并 metadata 默认、WebUI `command_perm_ui` 数据结构 |
| `src/common/features/cmd_perm/menu_display.py` | `raw_trigger_condition`、`effective_permission_avail_text`、帮助用权限文案 |
| `src/common/console/webui/env_sections.py` | `cmd_perm` 配置段与 payload 附加字段 |
| `src/common/features/cmd_perm/declare.py` | `command_perm_row` / `command_perm_list` 声明辅助 |
| `src/common/features/cmd_perm/metadata_defaults.py` | `PLUGIN_HOMEPAGE`、`PLUGIN_EXTRA_VERSION`、`PLUGIN_MENU_TEMPLATE` |
| `src/common/features/cmd_perm/metadata_text.py` | `usage_line`、`join_usage`、`SCENE_*` |
