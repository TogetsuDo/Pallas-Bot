...
| 项 | 说明 |
|----|------|
| **`.env` 文件** | 各进程读取同一份仓库 `.env`（数据库、`HOST`、插件配置等全局一致） |
| **按进程覆盖** | `run_sharded_bot.sh` 用 `env KEY=val` 设置 `PALLAS_BOT_ROLE`、`PALLAS_SHARD_ID`、`PORT` 等，**优先于 `.env` 同名项** |
...
示例编排见仓库 [`docker-compose.shard.example.yml`](../../docker-compose.shard.example.yml)（hub + 多个 worker，**共用** `./pallas-bot/.env` 与 `./pallas-bot/data`）。
