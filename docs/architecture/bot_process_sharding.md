...
- **接话 / 主动发言**：ingress 为 **shard 级 claim**（每条消息一个 worker 片通过，该片上各牛可进 matcher）；复读可接话时由 `repeater_fanout` 协调**全舰队在线牛各发一条**（跨片经 `bot_action`）。主动发言仍为定时任务，各 worker 本地 `MessageStore` 仍只收录本片 ingress 通过后的消息。
