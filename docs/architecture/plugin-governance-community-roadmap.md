...
| 变更 | 说明 |
| --- | --- |
| `validate_community_plugin_dir` | 增加 L1 规则：有口令型 matcher 时 warn 缺 `command_permissions`；`menu_data.command_permission` 与声明 ID 交叉校验 |
| `check --profile L1\|L2` | 输出画像档位与 capabilities 摘要 JSON |
...
- [ ] `uv run python tools/community_plugin_author.py check ./my_plugin --profile L1` 通过
