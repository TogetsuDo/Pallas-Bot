...
Bot / AI / WebUI 三仓 replay 与 tool contract 链路已实现；下列 Task Step 已全部勾选。回归命令：

```bash
# Bot
cd Pallas-Bot && uv run pytest tests/features/test_llm_tool_contracts.py tests/features/test_llm_runtime_debug.py tests/plugins/pb_webui/test_llm_history_api.py -q
# AI
cd Pallas-Bot-AI && uv run pytest tests/providers/test_tool_loop.py tests/providers/test_tool_schema.py tests/test_api_llm_replay.py -q
# WebUI
cd Pallas-Bot-WebUI && npm run build:ci
```

首版 replay 默认 `mock_tools`，不真实重放带副作用工具。
...
- [x] **Step 1: 新建 contract 模型，区分本地可执行定义与 transport 快照**
...
- [x] **Step 2: 扩展 `LlmToolSpec` 的静态元数据，但不要把 handler 混进 transport 对象**
...
- [x] **Step 3: 补一个从 `LlmToolSpec` 到 `ToolCatalogEntry` 的转换函数**
...
- [x] **Step 4: 让执行结果统一包成 `ToolResultEnvelope`，旧返回结构继续兼容**
...
- [x] **Step 5: 为 contract 加 roundtrip 测试**
...
- [x] **Step 1: 新增 `tool_catalog_for_chat()`，不要再只返回裸 `tool_schemas`**
...
- [x] **Step 2: 让现有 `tool_openai_schemas()` 从 `tool_catalog` 派生，而不是重复遍历 registry**
...
- [x] **Step 3: 在 `client.py` metadata 里同时写入新旧字段**
...
- [x] **Step 4: 给 `runtime_debug` 预留 request debug 开关**
...
- [x] **Step 5: 为兼容层写测试，确认 AI 尚未改造前请求仍可用**
...
- [x] **Step 1: 新建 AI 侧 trace 模型，保留 `rounds` 兼容现有 WebUI**
...
- [x] **Step 2: 优先从 `tool_catalog` 解析 schema，旧 `tool_schemas` 只兜底**
...
- [x] **Step 3: 在 `tool_loop.py` 里把裸 dict trace 换成模型驱动**
...
- [x] **Step 4: 每轮工具执行后补 `StageTrace` 与 `ToolCallTrace`**
...
- [x] **Step 5: 把 `plan` 与 `generate` 也记录成阶段，不只记录 `rounds`**
...
- [x] **Step 6: 更新测试，断言新 trace 字段存在且旧 UI 字段不回退**
...
- [x] **Step 1: 新建轻量落盘 helper，沿用 `jsonl` 风格**
...
- [x] **Step 2: 在 Bot 提交 AI 请求前写 request snapshot**
...
- [x] **Step 3: 在 `client.py` 把 `request_snapshot_id` 写回 metadata 给 AI 使用**
...
- [x] **Step 4: callback 收到 `agent_trace` 后单独落 runtime trace**
...
- [x] **Step 5: 在 `extended_api.py` 提供查询与导出接口**
...
- [x] **Step 6: 给 API 写测试，至少断言 request snapshot id 串起来**
...
- [x] **Step 7: 运行 Bot 侧 callback / API 测试**
...
- [x] **Step 1: 扩展前端类型，给 `agent_trace` 增加 `version` / `stages` / `request_snapshot_id`**
...
- [x] **Step 2: 在 `AiHistoryPage.vue` 增加 stage 摘要渲染**
...
- [x] **Step 3: 增加“复制 replay payload”按钮，先只调导出 API**
...
- [x] **Step 4: UI 上优先展示 `request_snapshot_id` 与 stage 列表，不新增复杂入口**
...
- [x] **Step 1: 更新现有内部文档，把新 contract 名称与 replay 边界写回**
...
- [x] **Step 2: 运行 Bot 仓 lint / format / targeted tests**
...
- [x] **Step 3: 运行 AI 仓 lint / format / targeted tests**
