...
实现 `ContextRepository` Protocol（`src/db/repository.py`），对 repeater 透明替换 `make_context_repository()` 的默认返回值。
...
| 路径 | 说明 |
|------|------|
| `src/corpus/` | config、merge、composite、community HTTP、enroll、status |
| `src/webui/corpus_federation_section.py` | WebUI「语料联邦」配置段 |
...
| 路径 | 说明 |
|------|------|
| `src/db/__init__.py` | `make_context_repository()` 委托 composite |
| `src/config/repo_settings.py` | flatten `[corpus]`、`[community_stats]` |
