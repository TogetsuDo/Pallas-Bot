...
- **胜负惩罚**：时长、败/胜者名片、双牛局消息替换；人类局败者仅在惩罚开始时代发一次噪音文案（不撤回后续发言，避免对群管撤消息失败）
- **流程**：总幕数（默认 5）、多 Bot 抢答冷却、幕间随机停顿、紧凑发群
- **事件与 QTE**：公共幕权重、QTE 事件权重倍率、兵刃幕额外 QTE 概率、兵刃随机抢攻/乱入抢认概率
...
| 人类，且**处理决斗的牛**为群管 | 败者改名片；惩罚开始时 @败者 代发一次噪音文案（不再逐条撤回消息） |
...
**抢答博弈**：关键词 QTE 可对对手伤害改为双方抢攻；兵刃随机 QTE、干员乱入亦可按配置掷「抢攻/抢认」。撰写与权重见 [event_packs/README.md](../../../src/plugins/duel/event_packs/README.md)。
