...
1. **联机漂流队列**：他群传来的图或文（图受「本会话已发图」上限约束，见下）。**每个 tick 仅按配置概率尝试从队列取一条**；未取到则本 tick 不消费他群实时句，更多机会留给历史/已学句。
2. **历史梦抽样**：`message` 里 `keywords` 以 `is_dream` 为前缀、近 **90 天**（`history_bottle` 内写死，与下方「梦库清理」保留天数独立）、**本进程内各在线 Bot 的 `bot_id` 并集**（同一库、多账号共享）；优先抽**非本群**，没有再抽上述并集内全库。
3. **已学句**：从复读侧 `Context` 等库随机取一条非 CQ 前缀的短句，昵称用随机展示名；重试次数可配，便于做梦初期语料少时仍较易出句。
4. **归档图**：按配置概率尝试拉取 [`pallas_image`](../pallas_image/README.md) 本地归档的成功出图。
...
- **梦库过期清理**：按 cron（默认每天 **4:00**）删除 `keywords` 以 `is_dream` 开头且 `time` 早于「当前时间 − 保留天数」的记录；Mongo 与 PostgreSQL 均支持。实现见 [`library_cleanup.py`](../../../src/plugins/dream/library_cleanup.py)。
...
| 字段 | 默认 | 说明 |
|------|------|------|
| `dream_drift_queue_tick_probability` | `0.28` | 每轮 worker tick 尝试从他群漂流队列取一条的概率；调低可减少他人实时句占比。 |
| `dream_archive_image_probability` | `0.26` | 走到归档分支时尝试发一张归档画的概率。 |
| `dream_echo_resample_attempts` | `22` | 已学句抽样最大重试次数；调高更容易在本 tick 发出一条已学句。 |
| `dream_hist_resample_attempts` | `12` | 历史梦抽样最大重试次数。 |
| `dream_library_cleanup_cron_hour` / `dream_library_cleanup_cron_minute` | `4` / `0` | 梦库清理定时任务时刻。 |
| `dream_message_retention_days` | `90` | 清理时保留最近多少天内的 `is_dream` 记录；更早的删除。 |
