...
默认部署为**单进程**（`bot.py` / `nb run`），不启用分片与 message_scrub 模板。
...
| `message-scrub` | `uv sync --extra message-scrub` | `uv run python tools/apply_deploy_profile.py message-scrub` | 入站消息审查 |
...
# 重启 Bot，在 WebUI「通用配置 → 消息审查」配置词表或 API
```
...
- [`message-scrub/`](message-scrub/README.md) — 审查开关与示例键
